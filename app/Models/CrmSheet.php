...
class CrmSheet extends Model
{
    ...
    protected $fillable = [
        'title', 'data', 'owner_id', 'folder_id', 'status',
        'is_uploaded', 'original_filename', 'stored_path', 'mime_type', 'file_size',
    ];

    protected $casts = ['data' => 'array', 'is_uploaded' => 'boolean'];

    public function owner() { return $this->belongsTo(User::class, 'owner_id'); }
    public function folder() { return $this->belongsTo(CrmFolder::class, 'folder_id'); }
    public function permissions() { return $this->hasMany(CrmSheetPermission::class, 'sheet_id'); }
