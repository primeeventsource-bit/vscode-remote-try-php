...
/**
 * Nexio / CMG Technologies statement parser.
 */
class NexioStatementParser extends KurvStatementParser
{
    public function version(): string { return '2.0.0'; }
    public function slug(): string { return 'nexio'; }

    protected function parseHeader(string $text): array
    {
        $header = $this->emptyResult()['header'];
        $header['processor_name'] = 'Nexio';

        if (preg_match('/(?:Merchant|DBA|Business)\s*[:\-]?\s*(.+)/i', $text, $m)) {
            $header['merchant_name'] = trim($m[1]);
        }
        if (preg_match('/(?:Merchant\s*(?:#|No|Number)|MID|Account\s*ID)\s*[:\-]?\s*([\d\-\*]+)/i', $text, $m)) {
